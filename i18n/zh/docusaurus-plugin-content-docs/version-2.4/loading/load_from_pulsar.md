...
提交 Routine Load 导入作业以后，您可以执行 [SHOW ROUTINE LOAD](../sql-statements/data-manipulation/SHOW%20ROUTINE%20LOAD.md) 语句来查看导入作业的信息。例如，您可以通过如下命令查看一个名为 `routine_wiki_edit_1` 的导入作业的信息：
...
修改前，您需要先执行 [PAUSE ROUTINE LOAD](../sql-reference/data-manipulation/PAUSE%20ROUTINE%20LOAD.md) 暂停导入作业。然后执行 [ALTER ROUTINE LOAD](..sql-reference/data-manipulation/alter-routine-load.md) 语句，修改导入作业的参数配置。修改成功后，您需要执行 [RESUME ROUTINE LOAD](../sql-reference/data-manipulation/RESUME%20ROUTINE%20LOAD)，恢复导入作业。然后执行 [SHOW ROUTINE LOAD](../sql-statements/data-manipulation/SHOW%20ROUTINE%20LOAD.md) 语句查看修改后的导入作业。
