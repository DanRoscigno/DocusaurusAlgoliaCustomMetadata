...
If you disable the query rewrite feature, CelerData adopts the regular query plan.
...
- **Check whether the refresh task exceeds the memory limit.**
  ...
  - Enable the Spill to Disk feature for the refresh task. CelerData supports spilling the intermediate results to disks when refreshing a materialized view. Execute the following statement to enable Spill to Disk:
...
You can define the following properties (session variables) while creating the materialized view or adding them using ALTER MATERIALIZED VIEW.
...
- **Check whether the  materialized view  is partitioned.**
  ...
  An unpartitioned materialized view can be costly to refresh because CelerData refreshes the whole materialized view each time. To solve this problem, you need to specify a partitioning strategy for the materialized view to refresh one partition each time.
...
- **Check whether the  materialized view  and the  query  match.**
  ...
  - CelerData matches the materialized view and the query with a structure-based matching technique rather than text-based matching. Therefore, it's not guaranteed that a query can be rewritten just because it appears similar to that of a materialized view.
...
- **Check whether the  materialized view  state is active.**
  ...
  CelerData checks the status of the materialized view before rewriting queries. Queries can be rewritten only when the materialized view state is active. To solve this problem, you can manually set the materialized view state to active by executing the following statement:
...
- **Check whether the  materialized view  meets the data  consistency  requirements.**
  ...
  CelerData checks the consistency of data in the materialized view and in the base table data. By default, queries can be rewritten only when the data in the materialized view is up-to-date. To solve this problem, you can:
