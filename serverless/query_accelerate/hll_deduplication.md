...
* Use the hash function [murmur_hash3_32](../sql-reference/sql-functions/hash-functions/murmur_hash3_32.md) to calculate the hash value of col2 as a 32-signed integer.
...
1. To use HyperLogLog de-duplication, you need to set the target indicator column type to ‘HLL’ and the aggregation function to [HLL_UNION](../sql-reference/sql-functions/aggregate-functions/hll_union.md) in the table creation statement.
2. Currently, only the aggregation table supports HLL as indicator column type.
3. When using `count distinct` on columns of the HLL type, CelerData will automatically convert it to the [HLL_UNION_AGG](../sql-reference/sql-functions/aggregate-functions/hll_union_agg.md) calculation.
...
* To find the total uv, run the following command:
