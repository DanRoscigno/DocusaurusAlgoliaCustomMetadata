...
- You can create bitmap indexes for all columns of a table that uses the Duplicate Key table or Unique Key table. For an Aggregate table or Primary Key table, you can only create bitmap indexes for key columns.
...
- Create a bitmap index for a column when you create a table. Example:
    ...
    For other parameter descriptions of the CREATE TABLE statement, see [CREATE TABLE](../sql-reference/sql-statements/data-definition/CREATE_TABLE.md).
...
![bitmap index](../assets/3.6.1-2.png)
...
After the query is sent, CelerData searches for the dictionary to get the coded value of `male`, which is `1`and then gets the bitmap of `male`, which is `0001`. This means that only the fourth row matches the query condition. Then CelerData will skip the first three rows and read only the fourth row.
