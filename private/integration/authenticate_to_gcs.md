...
CelerData supports using one of the following authentication methods to access Google Cloud Storage (GCS):
...
CelerData can authenticate to GCS in the following scenarios:

- Batch load data from GCS.
- Back up data from and restore data to GCS.
- Query Parquet and ORC files in GCS.
- Query [Hive](../query_data/hive_catalog.md), [Iceberg](../query_data/iceberg_catalog.md), [Hudi](../query_data/hudi_catalog.md), and [Delta Lake](../query_data/deltalake_catalog.md) tables in GCS.

In this topic, [Hive catalog](../query_data/hive_catalog.md), [file external table](../query_data/file_external_table.md), and [Broker Load](../sql-reference/sql-statements/data-manipulation/BROKER_LOAD.md) are used as examples to show how CelerData integrates with GCS in different scenarios. For information about `StorageCredentialParams` in the examples, see the "[Parameters](#parameters)" section of this topic.

> **NOTE**
>
> CelerData supports loading data or directly querying files from GCS only according to the gs protocol. Therefore, when you load data or query files from GCS, you must include `gs` as a prefix in the file path.
...
Use the [LOAD LABEL](../sql-reference/sql-statements/data-manipulation/BROKER_LOAD.md) statement to create a Broker Load job whose label is `test_db.label000`, in order to batch load data from GCS into the CelerData table `target_table`:
...
If your CelerData cluster is deployed on a VM instance hosted on Google Cloud Platform (GCP) and you want to use that VM instance to authenticate GCS, configure `StorageCredentialParams` as follows:
...
If your CelerData cluster is deployed on a VM instance hosted on GCP and you want to make that VM instance impersonate a service account, so as to make CelerData inherit the privileges from the service account to access GCS, configure `StorageCredentialParams` as follows:
...
If you want to make a service account (temporarily named as meta service account) impersonate another service account (temporarily named as data service account) and make CelerData inherit the privileges from the data service account to access GCS, configure `StorageCredentialParams` as follows:
