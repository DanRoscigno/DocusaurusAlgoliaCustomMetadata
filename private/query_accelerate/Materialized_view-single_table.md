...
Synchronous materialized views in CelerData can be created only on a single base table from [the default catalog](../query_data/default_catalog.md). They are essentially a special index for query acceleration rather than a physical table like asynchronous materialized views.

CelerData provides asynchronous materialized views, which supports creation on multiple tables and more aggregation operators. For the usage of **asynchronous materialized views**, see [Asynchronous materialized view](./Materialized_view.md).
...
- **Base table**
  ...
  For CelerData' synchronous materialized views, base tables must be a single native table from the [default catalog](../query_data/default_catalog.md). CelerData supports creating synchronous materialized views on Duplicate Key table, Aggregate tables, and Unique Key tables.
...
> - Currently, CelerData does not support creating multiple synchronous materialized views at the same time. A new synchronous materialized view can only be created when the previous one is completed.
