...
`loads` provides the results of load jobs. Currently, you can only view the results of [Broker Load](../../sql-reference/sql-statements/data-manipulation/BROKER_LOAD.md) and [INSERT](../../sql-reference/sql-statements/data-manipulation/INSERT.md) jobs from this view.

The following fields are provided in `loads`:

| **Field**            | **Description**                                              |
| -------------------- | ------------------------------------------------------------ |
| JOB_ID               | The unique ID assigned by CelerData to identify the load job. |
| LABEL                | The label of the load job.                                   |
| DATABASE_NAME        | The name of the database to which the destination CelerData tables belong. |
